...
% Note: acmart loads amsmath and amsfonts automatically
...
% ORCID support removed due to package availability issues
...
\contributionsbox{%
	\begin{enumerate}[itemsep=2pt,parsep=2pt]
		...
		\item[(3)] \textbf{Automated Policy Synthesis Pipeline}: LLM-driven translation of natural language principles to executable policies with \textbf{99.92\%} reliability for safety-critical applications through quintuple-model validation. This includes formal verification for 52.8\% of safety-critical rules amenable to SMT solvers (achieving 94.67\% success on this subset) and comprehensive multi-tier validation for all principles (\Cref{subsec:experimental_setup}).
...
\begin{algorithm}[!htbp]
	...
	\begin{algorithmic}[1]
		...
		\State Select enforcement strategy via WINA insights analysis (see \Cref{subsec:system_architecture} and \Cref{subsubsec:wina_performance_evaluation} for WINA components and strategies)
		\State Filter and optimize policies using WINA relevance scoring (see \Cref{subsubsec:wina_performance_evaluation})
		\State Verify constitutional compliance with ConstitutionalWINAIntegration (see \Cref{subsubsec:wina_performance_evaluation})
...
\begin{figure}[htbp]
	\centering
	\fbox{\parbox{0.9\columnwidth}{\centering \textbf{Appeal Workflow Diagram Placeholder} \\ \footnotesize This diagram would visually represent the multi-stage appeal process: Appeal Submission $\rightarrow$ Ombudsperson Triage (1-2 days) $\rightarrow$ [Optional Quick Fix] OR Technical Review (3-5 days) $\rightarrow$ [Optional Resolution] OR Escalation to Council Sub-committee (5-10 days) $\rightarrow$ [Optional Resolution/Recommendation] OR Full Council Review (10-20 days) $\rightarrow$ Final Decision \& Implementation. All stages log to an audit trail. The visual would use standard flowchart symbols and ensure clear text labels for each stage and decision point, designed to be colorblind-safe.}}
...
\begin{figure}[htbp]
	...
	\fbox{\parbox{0.9\columnwidth}{ % Enhanced mockup with examples
			\centering
			\textbf{Explainability Dashboard Interface Mockup:}\\
			\footnotesize
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}l>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.2cm}>{\centering\arraybackslash}p{1.1cm}@{}}
		...
		\bottomrule
	\end{tabular}
	\Description{Table showing Bias Detection Performance Across Domains (Financial Portfolio, Autonomous Vehicles, Neural Architecture) and Overall. Metrics: Bias Detection (\%), False Positive Rate (\%), Fairness Violation Detection (\%), Human Review needed (\%). Financial Portfolio: 91.2% bias detect, 8.3% false pos, 94.7% viol detect, 23.1% human rev. Autonomous Vehicles: 88.7% bias detect, 11.2% false pos, 89.4% viol detect, 19.8% human rev. Neural Architecture: 82.4% bias detect, 15.1% false pos, 85.2% viol detect, 16.7% human rev. Overall: 87.4% bias detect, 11.5% false pos, 89.8% viol detect, 19.9% human rev.}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}l>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.2cm}@{}}
		...
		\midrule
		Constitutional Compliance (\%) & 31.7±5.4                  & 59.9±9.6                   & 68.7±7.6\textsuperscript{a} & \textbf{\tablenumfmt{94.9}±3.2}  \\
		Adaptation Time (generations)  & N/A\textsuperscript{b}    & 15.2±12.3                  & N/A\textsuperscript{c}      & \textbf{\tablenumfmt{8.7}±2.1}   \\
...
\begin{table}[htbp]
	...
	\begin{minipage}{\linewidth}\footnotesize $^{\dagger}$Overall fairness score computed as weighted average across domains 2-5 only (domains with protected attributes). Domain 1 (Arithmetic) was excluded per the domain-appropriate evaluation framework.\end{minipage}
\end{table}
...
\Description{Example LLM prompt for synthesizing a Rego policy from constitutional principle CP-SAFETY-001 ("No Division Operator"). The prompt specifies the principle text, desired Rego rule name (`deny_division`), package name, expected output (Rego code, explanation, confidence score), and the condition for denial (input string contains '/').}
...
\hline
\end{tabular}
\Description{Extended domain evaluation results showing constitutional compliance, performance retention, enforcement latency, and fairness scores across five evaluation domains.}
\end{table}
...
\begin{enumerate}
	...
	\item \textbf{Embedding Fairness}: The framework explicitly integrates algorithmic fairness principles (\Cref{subsec:policy_synthesis_enforcement}) and bias detection mechanisms (\Cref{subsubsec:bias_detection_evaluation}) into the evolutionary process.
...
This appendix contains detailed technical specifications, algorithms, and experimental data supporting the main paper.

\subsection{Data Structures and Implementation Details}

Complete Python dataclass implementations for \texttt{ConstitutionalPrinciple} and \texttt{OperationalRule} with full field specifications, validation methods, and serialization protocols are available in the supplementary code repository.

\subsection{Experimental Artifacts and Reproducibility}

Complete experimental scripts, statistical analysis code, evaluation datasets, and detailed reproducibility instructions are provided. All artifacts follow FAIR principles and are available through the designated Zenodo repository with DOI references.
...
The safety checking algorithm employs static analysis techniques to detect potential safety violations in generated policies:

\begin{algorithm}[H]
\caption{Safety Pattern Detection}
\begin{algorithmic}[1]
\Require Policy rule $r$, safety patterns $\mathcal{P}_{safe}$
\Ensure Safety assessment $s \in \{SAFE, UNSAFE, UNKNOWN\}$
\Function{CheckSafety}{$r$, $\mathcal{P}_{safe}$}
\For{each pattern $p \in \mathcal{P}_{safe}$}
    \If{$r$ matches anti-pattern $p$}
        \State \Return UNSAFE
    \EndIf
\EndFor
\State \Return SAFE
...
The conflict detection algorithm identifies logical inconsistencies between policies:

\begin{algorithm}[H]
\caption{Policy Conflict Detection}
\begin{algorithmic}[1]
\Require New rule $r_{new}$, existing rules $\mathcal{R}_{existing}$
\Ensure Conflict assessment $c \in \{CONFLICT, NO\_CONFLICT\}$
\Function{DetectConflicts}{$r_{new}$, $\mathcal{R}_{existing}$}
\For{each rule $r \in \mathcal{R}_{existing}$}
    \If{logical contradiction detected between $r_{new}$ and $r$}
        \State \Return CONFLICT
    \EndIf
\EndFor
\State \Return NO\_CONFLICT
\EndFunction
\end{algorithmic}
\end{algorithm}
...
The empirical Lipschitz constant estimation follows a systematic protocol:
