...
\usepackage{cleveref} % For smart cross-referencing (e.g., \Cref{fig:myfig})
...
\begin{enumerate}[itemsep=2pt,parsep=2pt]
  ...
  \item[(3)] \textbf{Automated Policy Synthesis Pipeline}: LLM-driven translation of natural language principles to executable policies with 73--93\% success rates, including formal verification for safety-critical rules and multi-tier validation (\Cref{sec:synthesis_evaluation}).
...
\begin{table}[h]
  ...
  \begin{tabularx}{\columnwidth}{@{}p{1.8cm}X@{}}
    ...
    \midrule
    \tablenumfmt{ACGS} & AI Constitution Generation System \\
    \tablenumfmt{AC} & Artificial Constitution Layer \\
    \tablenumfmt{CAI} & Constitutional AI \\
    \tablenumfmt{EC} & Evolutionary Computation \\
    \tablenumfmt{GS Engine} & Self-Synthesizing Engine (LLM-based policy generator) \\
...
\Function{SynthesizeRule}{$p$, $\mathcal{C}$, $\mathcal{F}$}
  ...
  \ForAll{$r_{\text{candidate}} \in \mathcal{R}_{\text{candidates}}$} \Comment{Multi-tier validation pipeline}
    ...
    \If{$\text{semantic\_score} < \tau_{\text{semantic}}$} \Comment{$\tau_{\text{semantic}} = 0.85$ threshold}
    ...
    \If{$\text{bias\_assessment}.\text{risk\_score} > \tau_{\text{bias}}$} \Comment{$\tau_{\text{bias}} = 0.7$ threshold}
...
\begin{itemize}
    ...
    \item \textbf{Fairness Metrics}: Demographic parity, equalized odds, calibration across protected attributes
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Hardware}: 32-core Intel Xeon, 128GB RAM, evaluation parallelized across 8 nodes
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Regression Analysis}: Robust regression with Huber estimator for scalability claims, reporting $R^2$, adjusted $R^2$, and residual analysis
    ...
    \item \textbf{Effect Size Reporting}: Cohen's $d$ for practical significance assessment, with interpretation guidelines (small: 0.2, medium: 0.5, large: 0.8)
...
\begin{table}[htbp]
  \centering
  \caption{\textbf{Bias Detection Performance Across Domains.} Systematic bias detection identifies potentially discriminatory policies with high accuracy.}
  ...
  \begin{tabular}{@{}l>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.2cm}>{\centering\arraybackslash}p{1.1cm}@{}}
    \toprule
    \tableheader{Domain} & \tableheader{Bias Detect. (\%)} & \tableheader{False Pos. (\%)} & \tableheader{Fair. Viol. (\%)} & \tableheader{Human Rev. (\%)} \\
    \midrule
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}l>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.1cm}>{\centering\arraybackslash}p{1.2cm}@{}}
    ...
    \midrule
    Compliance (\%)         & 31.7±5.4   & 59.9±9.6       & 68.7±7.6     & \textbf{\tablenumfmt{94.9}±3.2}   \\
    Adapt. Time (gen)   & N/A               & 45.2±12.3      & N/A                 & \textbf{\tablenumfmt{8.7}±2.1}    \\
...
\begin{itemize}
    \item \textbf{PGC Latency}: 50,000 independent measurements across domains with Welch's t-test confirming significant performance improvement over baseline OPA ($p < 0.001$, Bonferroni corrected)
    \item \textbf{Synthesis Success Rates}: Wilson score confidence intervals with Chi-square tests revealing significant differences between principle complexity levels ($\chi^2 = 23.47, p < 0.001$)
    \item \textbf{Constitutional Compliance}: ANOVA with post-hoc Tukey HSD tests showing significant improvements across all domains ($F(3,396) = 187.3, p < 0.001$)
\end{itemize}

\subsubsection{Effect Size Analysis}
All improvements demonstrate large practical significance:
\begin{itemize}
    \item \textbf{Compliance Improvement}: Cohen's $d = 3.2$ (very large effect)
    \item \textbf{Latency Reduction}: Cohen's $d = 2.8$ compared to manual rules
    \item \textbf{Adaptation Speed}: Cohen's $d = 4.1$ compared to manual approaches
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}l>{\centering\arraybackslash}p{0.9cm}>{\centering\arraybackslash}p{1.0cm}>{\centering\arraybackslash}p{1.0cm}>{\centering\arraybackslash}p{1.0cm}>{\centering\arraybackslash}p{1.0cm}@{}}
    ...
    \midrule
    \textit{Overall} & \textit{11.2} & \textit{\tablenumfmt{91.3}} & \textit{\tablenumfmt{77.0}} & \textit{\tablenumfmt{45.7}} & \textit{\tablenumfmt{8.3}/10} \\
    \bottomrule
  \end{tabular}
  \resettable
\end{table}
...
\begin{itemize}
    \item \textbf{GitHub Repository:} \url{https://github.com/soln-ai/alphaevolve-acgs} (MIT License, publicly available)
...
\Function{DetectPolicyBias}{$r$, $p$, $\mathcal{A}$}
  ...
  \If{$\beta.\text{risk\_score} > \tau_{\text{high\_risk}}$ \textbf{or} $p.\text{priority} \geq 9$}
    \State $\beta.\text{requires\_human\_review} \gets \text{True}$
    \State $\beta.\text{review\_priority} \gets \text{"HIGH"}$
  \ElsIf{$\beta.\text{risk\_score} > \tau_{\text{medium\_risk}}$}
    \State $\beta.\text{requires\_human\_review} \gets \text{True}$
...
\begin{lstlisting}[language=DOT, caption={Complete DOT language specification for the Appeal and Dispute Resolution Workflow. Compile using Graphviz to generate the flowchart shown in \Cref{fig:appeal_workflow}.}, label=lst:appeal_workflow_dot_appendix, basicstyle=\ttfamily\tiny, numbers=none]
